define("inno/en/table/2.0/table",function(require, exports, module){
	var $=require("$");
...
//			height:800,
		...
		drawYAxisTd:function ( tree,rowIndex ){
			...
			if( !tree.isDraw && !tree.isRoot){
				...
				if(this.attrs.yaxis.level - 1 != tree.level){
		...
		initialize:function (cfg){
			...
			if( this.attrs.splitTable ){
				...
				var marginleft = (rcontent.width()-rtile.width())/25;
				if(marginleft<0){
					marginleft=0;
				}
				rtile.css("marginLeft", marginleft +"px" );
				var tblReportContent = this.attrs.content.find(".tblReportContent");
				var tlength = tblReportContent.width();
				if( tlength >= this.attrs.congelationTitle){ // 超过极限宽度则出滚动条
...
//				table.find(".tblReport_msg_con").html( title );
		...
		congelationTitle:function( html ){
			...
			parent.css( {overflowY:"auto",overflowX:"auto",position:"relative",zIndex:"899"} );
			var pwidth = this.attrs.width;
			var pheight = this.attrs.height;
			if( !pwidth ){
				var wwidth=$(window).width();
				var wheight=$(window).height();
//				if( wheight > parent.find(".tblReport").height() ){
//					wheight = parent.find(".tblReport").height();
//				}
//				if( wwidth > parent.find(".tblReport").width() ){
//					wwidth = parent.find(".tblReport").width();
//				}
			...
			title.css({"position":"absolute","z-index":99,"top":"0px","left":"0px"});
			xtitle.css({"position":"absolute","z-index":97});
			ytitle.css({"position":"absolute","z-index":97});
			...
			xtitle.css({"top":"0px","left":(width+pright+pleft)+"px","display":"none"});
			ytitle.css({"top":height+"px","left":"0px","width":(width+pright+pleft+1)+"px"});
			if( pwidth ){
...
define("inno/en/table/2.0/table2",function(require, exports, module){
	var $=require("$");
...
define("inno/en/table/2.0/table-debug",function(require,exports, module){
	var $=require("$");
	var Table=require("./table");
	...
	Bill={
									...
									for(var i =0;i< nodes.length;i++){
										...
										if( node.text ){
											...
											if(text.indexOf("亿立方米")>=0){
												...
												}else if( title.iname == "化石燃料低位发热值(KJ/KG)" ){
													text = text.replace("（亿立方米）","(TJ/亿立方米)");
												}else if( title.iname == "移动源氧化亚氮排放因子(KG/TJ)" ){
													text = text.replace("（亿立方米）","(KG/TJ)");
												}else if( title.iname == "移动源甲烷排放因子(KG/TJ)" ){
													text = text.replace("（亿立方米）","(KG/TJ)");
												}else if( title.iname == "CH4(吨)" ){
													text = text.replace("（亿立方米）","(吨)");
												}else if( title.iname == "N2O(吨)" ){
													text = text.replace("（亿立方米）","(吨)");
												}else if( iunit == "KJ/KG" ){
		...
		getIcmeta:function(){
			var _this = this;
			var parameter={
			     url:address + "bill/common/get/icmeta/instance",
		...
		getValue:function(){
			var _this = this;
			var parameter={
			     url:address + "bill/common/get/value",
			     data:{ rmodelId : _this.attrs.tableId },
			     success:function(data){
		...
		showValue:function(data){
			var _this = this;
	 		...
	 		$.each(data,function(n,value) { 
	 			num++;
	 			var sign=value.inputOrCalSign;//填写或者公式标识
	 			var rid=value.riCMapModelId;//单元格ID
	 			var gid=value.icDetailIdGroup;//维度组合
	 			var comment=value.icValueComment || "";//单元格备注信息
	 			var icValueTips = value.icValueTips||"";
	 			var srcsel = value.icValueSrcSel || "";//单元格类型
	 			var hjtype = value.icMetaMapCalInSum;			
	 			if(comment=="null"){
	 				comment="";
	 			}
	 			...
	 			id = id.split("(").join('\\(');
	 			id = id.split(")").join('\\)');
	 			var imetaId=value.imetaId;
	 			var romdelId = value.rmodelId.split("|").join('\\|');
	 			var td = $("#"+romdelId).find("td[gid="+id+"][x="+imetaId+"]");
	 			td.attr("icid",rid);
	 			var val=value.icValue || "";
	 			if(val.indexOf(".")==0){
	 				val="0"+val;
	 			}
	 			if(val.indexOf("-.")==0){
	 			...
	 			if("CAL"== sign){
	 				if("JS"== srcsel){
	 					td.attr("title","数值："+val+"\n说明："+icValueTips);
	 					if(_this.attrs.tableType == "CAL"){
	 						val = Math.round(val) || "0";
	 						val = _this.Test(val);
	 					}else{
	 						val = _this.changeVal(val);
	 					}
	 					if(val){
	 						val = "<span>"+val+"</span>";
	 					}else{
	 						val = "<span style = 'visibility:hidden' ></span>";
	 					}
	 					val=val+"<img class='detailImgs  carbonreport-td-testimg' src='"+address+"commons/imgs/JS.png' >";
	 					tdclass="calTd";
	 				}else{
	 					val="";
	 					td.attr("title","请选择一种数据类型");
	 				}
	 			}else if("INPUT"==sign){
	 				var x=td.attr("xaxis");
	 				var y=td.attr("yaxis");
	 				if("QS"==srcsel){
	 					td.attr("title","数值："+val+"\n说明："+icValueTips);
	 					val=_this.changeVal(val);
	 					val="<span>"+val+"</span>"+"<img class='detailImg carbonreport-td-testimg' src='"+address+"commons/imgs/QS.png'>";
	 					tdclass="qsTd";
	 				}else if("JC"==srcsel || "QT"==srcsel || "GS"==srcsel){
	 					td.attr("title","数值："+val+"\n说明："+icValueTips);
	 					tdclass="";
	 					val="<input txy='"+x+""+y+"' class='edit carbonreport-inputs' value='"+val+"' edvalue='"+val+"' />";
	 					val="<span>"+val+"</span>"+"<img class='detailImg  carbonreport-td-testimg' src='"+address+"commons/imgs/" + srcsel + ".png' >";
	 				}else{
	 					td.attr("title","数值："+val+"\n说明："+icValueTips);
	 					val="<input txy='"+x+""+y+"' class='edit carbonreport-inputs' value='"+val+"' edvalue='"+val+"' />";
	 				}
	 				...
	 				td.attr("title","填写内容："+comment+"\n说明："+icValueTips);
	 				comment = comment.substring(0, 5);
	 				val="<span>"+comment+"</span>"+"<img class='detailImg  carbonreport-td-testimg' src='"+address+"commons/imgs/TX.png' >";
	 				tdclass="qsTd";
	 			}else if("CHOICE" == sign){
	 				td.attr("title","选择内容："+val+"\n说明："+icValueTips);
	 				val = val.substring(0, 5);
	 				val="<span>"+val+"</span>"+"<img class='detailImg  carbonreport-td-testimg' src='"+address+"commons/imgs/XZ.png' >";
	 				tdclass="qsTd";
	 			}
	 			td.html(val);
	 			td.addClass(tdclass);
	 			td.attr("I_C_VALUE_SRC_SEL",srcsel);
	 		});
		},
		getImeta:function(){
			var _this = this;
			var parameter={
			     url:address + "bill/common/get/imeta/instance",
			     data:{ rmodelId : _this.attrs.tableId },
			     success:function(data){
		...
		getLayoutCfg:function(){
			var _this = this;
			var parameter={
				 url: address + "bill/common/get/layout",
			     type:"POST",
			     data:{rmodelId:_this.attrs.tableId},
			     success:function(data){
		...
		changeVal:function(val){
			if( val.length>=9 ){
				...
				}else if(0<=val.indexOf(".")<9){
					val=val.substring(0, 9);
				}else if(9<=val.indexOf(".")){
					val=val.substring(0, 6)+"...";
				}			
			}
			return val;
