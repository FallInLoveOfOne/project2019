define("jquery/easyui/1.3.0/datagrid-detailview",["$"],function(require, exports, module){	
	 var $ = require('$');	
	...
	// 上下分页选择的时候专用
	selectIdView = $.extend({}, $.fn.datagrid.defaults.view, {
		render: function(target, container, frozen){
		var state = $.data(target, 'datagrid');
		var opts = state.options;
		if (frozen){
	...
	onAfterRender: function(target){
		...
		for(var selectId in selectIdJson){ // 遍历json对象中存储的id进行赋值勾选
    		var id = selectIdJson[selectId];
    		if(id != null && id.length > 0){
    			$("input[value="+id+"]").attr("checked",true); // 给选中过的任务打勾
    			$("input[value="+id+"]").parent().parent().parent().addClass("datagrid-row-selected"); // 给任务渲染上颜色
    			var trId = $("input[value="+id+"]").parent().parent().parent().attr("id");
    			if(trId != null &&trId.length > 0){
    				var trIdArr = trId.split('-');
        			var sumTrId = trIdArr[0] + '-' +trIdArr[1] + '-' +trIdArr[2] + '-1-' +trIdArr[4];
        			$("#"+sumTrId).addClass("datagrid-row-selected"); // 给任务渲染上颜色
    			}
    			
    		}
    	}
		if(selectIdJson['AllCheckbox'] == 'AllCheckbox'){ // 如果是相等的情况下则进行勾选
			$(".datagrid-htable input").attr("checked",true); // 给选中过的任务打勾
		}
		
		 // 全选点击
		$(".datagrid-htable input").click(function(){
			if($(this).attr("type") != 'checkbox'){ // 如果不是选择框则返回
    			return false;
    		}
			var inputObj = $(".datagrid-btable").find('input');
			if($(this).is(':checked')){ // 如果当前是选中状态
    			selectIdJson['AllCheckbox'] = 'AllCheckbox';
    			for(var index = 0;index < inputObj.length;index ++){
    				var inputVal = inputObj.eq(index).val()
    				selectIdJson[inputVal] = inputVal
    			}
    		}else{
    			selectIdJson['AllCheckbox']  = ''; // 否则设置为空
    			for(var index = 0;index < inputObj.length;index ++){
    				var inputVal = inputObj.eq(index).val()
    				selectIdJson[inputVal] = '';
    			}
    		}
		});
		
		// 下面的复选点击
    	$(".datagrid-btable tr").click(function(){
    		dataGridObj = $(this);
    		setTimeout(function(){
    			var inputObj = dataGridObj.find('input');
        		var selVal = inputObj.eq(0).val();
        		if(inputObj.eq(0).attr("type") != 'checkbox'){ // 如果不是选择框则返回
        			return false;
        		}
        		if(inputObj.eq(0).is(':checked')){ // 如果当前是选中状态
        			selectIdJson[selVal] = selVal
        		}else{
        			selectIdJson[selVal] = ''; // 否则设置为空
        		}
        		var isAllSel = true; // 当前是否全部选中了的标记
        		var inputObj = $(".datagrid-btable").find('input');
    			for(var index = 0;index < inputObj.length;index ++){
    				if(!inputObj.eq(index).is(':checked')){
    					isAllSel = false;
    				}
    			}
    			if(!isAllSel){ // 如果没有全部选中则将全选勾中的去掉
    				$(".datagrid-htable input").attr("checked",false); // 给选中过的任务打勾
    				selectIdJson['AllCheckbox'] = '';
    			}else{
    				$(".datagrid-htable input").attr("checked",true); // 给选中过的任务打勾
    				selectIdJson['AllCheckbox'] = 'AllCheckbox';
    			}
	...
	// 上下分页选择的时候专用(不带加号)
	selectIdViewNew = $.extend({}, $.fn.datagrid.defaults.view, {
		render: function(target, container, frozen){
			var state = $.data(target, 'datagrid');
			var opts = state.options;
			if (frozen){
			...
			var fields = $(target).datagrid('getColumnFields', frozen);
			var table = [];
			table.push('<table class="datagrid-btable" style="width:100%" cellspacing="0" cellpadding="0" border="0"><tbody>');
			for(var i=0; i<rows.length; i++) {
				// get the class and style attributes for this row
				var css = opts.rowStyler ? opts.rowStyler.call(target, i, rows[i]) : '';
				var classValue = '';
			...
			for(var i=0; i<fields.length; i++){
				...
				if (col){
					...
					if (col.checkbox){
						...
						//cc.push('<div style="text-align:center;width:16px;height:16px;">');
						cc.push('<span class="datagrid-row-expander datagrid-row-expand" style="display:none;width:0px;height:16px;cursor:pointer;" />');
						//cc.push('</div>');
					} else if (col.formatter){
						cc.push(col.formatter(value, rowData, rowIndex));
					} else {
