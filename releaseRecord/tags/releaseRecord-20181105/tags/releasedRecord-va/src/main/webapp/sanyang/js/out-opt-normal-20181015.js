define(function(require){
    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            cols: [[
                {field:'first',width:'5%',title:'',templet: function(d){return getDaysFormat(d.DETOX_END,'')}},
            ...
            if(obj.event === 'detail'){// 查看按钮
        		...
        		if(("old"==isNewOpt)&&("processFail"!=nextStep)){// 如果有操作记录，直接跳到原来位置
        			...
        			if(parseInt(holdEnd)<=parseInt(nowDate)){
        				layer.confirm('是否确认在押人员基本信息，并在非法犯罪系统中完成出所相关流程？', {btn: ['确认完成', '尚未完成']},
        			        function(index){// 确认完成入口
        						layer.closeAll();
        			        	// 达到时间单的出所，【确认完成】，下一步
			        			var param = {
			            				url:address+"release/achieveTimeSureOut",
			            				type:"POST",
			            				data:{number:personNumber,name:userName,outType:'normal'},
			            				async:false,
			            				success:function(result){
			        		...
			        		function(){
			        			layer.confirm('尚未完成出所流程，是否继续出所？', {btn: ['暂不出所', '跳过继续下一步']},
		        			        function(){
					        			layer.closeAll();
					        			// 达到时间，【尚未完成】，暂不出所
					        			var param = {
					            				url:address+"release/uncompleteZB",
					            				type:"POST",
					            				data:{number:personNumber,name:userName,outType:'normal'},
					            				async:false,
					            				success:function(result){
					        		...
					        		function(){
					        			...
					        			// 达到时间，【尚未完成】，跳过、下一步
					        			var param = {
					            				url:address+"release/uncompleteNext",
					            				type:"POST",
					            				data:{number:personNumber,name:userName,outType:'normal'},
					            				async:false,
					            				success:function(result){
        			...
        			if(parseInt(holdEnd)>parseInt(nowDate)){
        				layer.confirm('尚未到达规定出所时间，是否进行临时出所？', {btn: ['是，临时出所', '否，正常出所']},
        			        function(){
			        			...
			        			// 未到达时间，临时出所、【是】
			        			var param = {
			            				url:address+"release/temOutY",
			            				type:"POST",
			            				data:{number:personNumber,name:userName,outType:'temp'},
			            				async:false,
			            				success:function(result){
			            					//window.open(address+"release/toFaceCheck?prisonId="+prisonId+"&number="+personNumber);
			            					window.open("release/toOutListUpload?prisonId="+prisonId+"&number="+personNumber);
...
//        self.parent.document.getElementById("iframe").src = "./iframe/release/normal.html";
        self.parent.document.getElementById("iframe").src = "release/toReleaseManage";
    })
    $('.out-temp').on('click', function(){
//        self.parent.document.getElementById("iframe").src = "./iframe/release/temp.html";
