...
//    private static final Random random = new Random();  
     ...
     * 发送心跳信息  
     */  
    public void sendLong(String param/*long param*/) {  
        try {  
            if (session.isOpen()) { 
            	count_ = count_+1;
                this.session.getBasicRemote().sendText(param+"统计发送值:"+count_/*String.valueOf(param)*/);  
            }  
        } catch (IOException e) {  
            try {  
                this.session.close();  
...
//            sendLong(param); 
        	String data = UUID.randomUUID().toString();//"{\"pageNum\":1,\"pageSize\":20,\"size\":2,\"orderBy\":null,\"startRow\":1,\"endRow\":2,\"total\":2}";
        	sendLong(data);
        }     
    };  
