...
$(window).resize(function() {
		...
		table.on('tool(last-report)', function(obj) {
			...
			if (obj.event === 'reportName') {
				layer.confirm('确认进入报告详情页面？', {
					...
					var inTime = dateFormat(data.inTime);
					var id = data.id;
					var name = data.name;
	...
	$('.QuickSearch').on('click', function() {
		...
		var checkedType = searchType;//$("input[name='serachType']:checked").val();
	...
	    params: function(){
	        var con = $.trim(this.prop.prev_value)||'';
