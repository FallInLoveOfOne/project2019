define(function(require){
    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            cols: [[
                {field:'first',width:'2%',title:'',templet: function(d){return getDaysFormat(d.DETOX_END,'')}},
...
//            	var userObj={number:personNumber,name:userName,outType:'normal'};
    ...
    function updatesbphoto(infoNumber){
    	var param = {
				url:address+"release/updateSB",//"release/achieveTimeSureOut",
				type:"POST",
				data:{number:infoNumber},
				async:false,
				success:function(result){
					if(result){
						var isup = result["isup"];
						if("true"==isup){
							alert("更新成功");
						}else{
							var xs = result["result"];
							if("false"==xs){
								alert("作为底图像素低");
							}
						}
