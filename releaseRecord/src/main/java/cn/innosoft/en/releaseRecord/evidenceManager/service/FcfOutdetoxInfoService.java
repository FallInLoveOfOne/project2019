...
import cn.innosoft.en.releaseRecord.releaseManager.service.TimeTaskService;
...
//		jcDetoxInfo.setKsUserId("");
	...
	public Map<String, Object> getYInfoByKsId(String ksId) {
		if(null==ksId||"".equals(ksId)){
			return null;
		}
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("ksId", ksId);
		List<Map<String, Object>> list = findMapBySql("fcfOutdetoxInfo-getInfoByKsId", params);
		if(null!=list&&list.size()>0){
			return list.get(0);
		}
		return null;
	...
	public void timeComeBack(String number) {
		...
		String setTime =  PropsUtil.getValue("autoComeBackTime");
		setTime = (null==setTime||"".equals(setTime)?"0":setTime);
		int backtime = Integer.valueOf(setTime);// 小时计
		if((BusinessConstant.TEMP_KEY.equals(outTypeVal))&&(!"0".equals(setTime))){
			Timer timer = new Timer();
			TimeTaskService timeTaskService = new TimeTaskService(number);
			timer.schedule(timeTaskService, backtime*60*60*1000);// 60*60*1000--1小时
		}
	}
